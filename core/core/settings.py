...
MIDDLEWARE = [
    ...
    # Serves the collected static files straight from Gunicorn/Uvicorn.
...
# WhiteNoise compresses and fingerprints the files that `collectstatic`
# writes to STATIC_ROOT. With DEBUG=True (local development) URLs stay
# un-hashed, so `collectstatic` is not needed to run the site locally.
STORAGES = {
    "default": {
